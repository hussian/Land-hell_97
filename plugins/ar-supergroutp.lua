...
local function check_member_super(cb_extra, success, result)
  ...
  if success == 0 then
	send_large_msg(receiver, "ابني👶🏻مو شغلك هذا ادعبل منها🚶")
  end
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = '😍شد جابولك بو9999ت شكاكي 😱'
...
local function check_member_superrem(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
	  local text = '☹️تم تعطيل المجموعة ميستاهلون 😱.'
      return reply_msg(msg.id, text, ok_cb, false)
    end
  end
end
...
local function lock_group_links(msg, data, target)
  ...
  return '🖕🏿 مزدودة الروابط ابني لاتلح 😡'
  else
    data[tostring(target)]['settings']['lock_link'] = 'yes'
    save_data(_config.moderation.data, data)
    return ' تم 👋 صار استاذي نقفلن الروابط 👮'
  end
end
...
local function unlock_group_links(msg, data, target)
  ...
  if group_link_lock == 'no' then
    return '✅ استاذي لاتصير غبي الروابط مفتوحة👋👮'
  else
    data[tostring(target)]['settings']['lock_link'] = 'no'
    save_data(_config.moderation.data, data)
    return ' تم 😂 استاذي الروابط والحمزة مفتوحة لح بعد  🖕🏿'
  end
end
...
local function lock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'yes' then
    return '👍 استاذي الكلايش مفتوحة 👋👮'
  else
    data[tostring(target)]['settings']['lock_spam'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'تم 👋 استاذي انفتحت الكلايش 👮'
  end
end
...
local function unlock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'no' then
    return '✅ استاذي لاتصير غبي مفتوحة الكلايش 👋 👮'

  else
    data[tostring(target)]['settings']['lock_spam'] = 'no'
    save_data(_config.moderation.data, data)
    return 'تم 👍استاذي نقفلن الكلايش 👋 👮'
  end
end
...
local function lock_group_flood(msg, data, target)
  ...
  if group_flood_lock == 'yes' then
    return '✅ استاذي التكرار مقفول  👋👮'
  else
    data[tostring(target)]['settings']['flood'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'تم 👋استاذي انقفل التكرار 👮'
  end
end
...
local function unlock_group_flood(msg, data, target)
  ...
  if group_flood_lock == 'no' then
    return '✅ استاذي لاتصير غبي التكرار مفتوح 👋👮'
  else
    data[tostring(target)]['settings']['flood'] = 'no'
    save_data(_config.moderation.data, data)
    return 'تم 👋 استاذي انفتح التكرار 👮'
  end
end
...
local function lock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'yes' then
    return '✅ استاذي الملصقات مقفولة والله 👋👮'
  else
    data[tostring(target)]['settings']['lock_sticker'] = 'yes'
    save_data(_config.moderation.data, data)
    return 'تم ✅ استاذي انقفلن الملصقات 👋👮'
  end
end
...
local function unlock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'no' then
    return '✅ استاذي لاتصير غبي مفتوحة الملصقات 👋👮'
  else
    data[tostring(target)]['settings']['lock_sticker'] = 'no'
    save_data(_config.moderation.data, data)
    return 'تم ✅استاذي انفتحن الملصقات 👋👮'
  end
end
...
local function promote2(receiver, member_username, user_id)
  ...
  data[group]['moderators'][tostring(user_id)] = member_tag_username
  save_data(_config.moderation.data, data)
  send_large_msg(receiver, member_username..' رفعوك ادمن بس  👮👋 لاتحس روحك عميد بالقوات القذرة 🌚🖕🏿.')
end
...
local function demote2(receiver, member_username, user_id)
  ...
  data[group]['moderators'][tostring(user_id)] = nil
  save_data(_config.moderation.data, data)
  send_large_msg(receiver, member_username..' هاااا شكلتلك لاتحس روحك 😂بالقوات القذرة شفت نزلت عضو 👮.')
end
		...
		if matches[1] == 'قفل' and is_momod(msg) then
			...
			local msg_type = ''
				if not is_muted(chat_id, msg_type..': yes') then
					...
					return "تمام "..msg_type.." 😈الصور مقفولة من قبل الجحيم😈"
				end
			end
